...
import api from '../api';
import { Trash2, Plus, Search, Filter, Upload, Download } from 'lucide-react';
...
const Dictionary = () => {
    ...
    const handleExport = async () => {
        try {
            setLoading(true);
            const response = await api.get('/admin/dictionary/export', {
                responseType: 'blob'
            });
            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', `dictionary_export_${new Date().toISOString().split('T')[0]}.xlsx`);
    ...
    return (
        <div className="p-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                ...
                <div className="card h-fit">
                    <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                        <Upload className="text-green-600" size={20} /> Import / Export
                    </h3>
                    <div className="flex flex-col gap-4">
                        <button
                            onClick={handleExport}
                            className="btn bg-green-50 text-green-700 hover:bg-green-100 border border-green-200 justify-center gap-2"
                        >
                            <Download size={18} /> Download All Words
                        </button>
